...
```
agent-smith/
├── commands/              72K  ✅ KEEP (active plugin)
├── templates/             64K  ✅ KEEP (source code)
├── venv/                  32K  ❌ REMOVE (old venv)
├── .claude/                ?   ❌ REMOVE (unrelated)
├── data/templates/         ?   ❌ REMOVE (runtime output)
├── .pytest_cache/          ?   ❌ CLEAN
├── .mypy_cache/            ?   ❌ CLEAN
├── .ruff_cache/            ?   ❌ CLEAN
├── htmlcov/              140K  ❌ CLEAN
├── agent_smith.egg-info/  56K  ❌ CLEAN
├── skill/scripts/        1.9M  ⚠️  BUILD ARTIFACT
└── [rest kept]
```
...
```
agent-smith/
├── commands/                   # Slash commands (KEPT - active)
├── templates/                  # Composable templates (KEPT - source)
│   ├── primary/
│   ├── living/
│   └── additional/
├── skill/                      # Skill source (scripts/ regenerated during build)
├── scripts/                    # Main source (1.9M)
├── data/                       # Working data (templates/ gitignored)
├── tests/                      # Test suite
├── docs/                       # Documentation
├── .claude-plugin/             # Plugin metadata
├── agent-smith.skill           # Packaged skill
...
3. 🔧 Update build script first (ensures scripts can be regenerated)
4. 🔧 Update .gitignore
5. 🧪 Test build: `./build-skill.sh`
6. 🗑️ Execute Phase 1: Remove redundant directories
7. 🗑️ Execute Phase 2: Clean build artifacts
8. ✅ Validate with checklist
9. 📝 Commit changes
10. 🧪 Final verification: rebuild and test skill
...
**Status**: ✅ CORRECTED & READY FOR APPROVAL
